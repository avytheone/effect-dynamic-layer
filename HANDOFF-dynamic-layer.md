...
**Current packaging override:** the “working name” and original no-publication condition for version 0.1 are retained below as historical evidence. The approved package identity is `@avytheone/effect-dynamic-layer` at version `0.1.0`, under the MIT license with copyright 2026 Alexey Yakimanskiy; the repository is now public. The owner has explicitly authorized the manual first publication of `0.1.0`, which remains pending until registry confirmation. npm Trusted Publisher is configured only after that bootstrap; later tag-driven releases and publisher-setting changes remain separately authorized operations. The exact release contract is maintained in [README.md](README.md) and [docs/status.md](docs/status.md).
