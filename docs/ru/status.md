...
Проект использует лицензию MIT, правообладатель — Alexey Yakimanskiy, 2026 год. После разрешённой проверки текущих файлов и доступной истории Git репозиторий `avytheone/effect-dynamic-layer` открыт; GitHub подтвердил видимость `PUBLIC`. **Текущее состояние выпуска (обновить после подтверждения в реестре):** владелец явно разрешил первую публикацию версии `0.1.0`; первоначальное создание пакета в реестре вручную ожидает подтверждения, а npm Trusted Publisher можно настроить только после его успешного завершения.
...
Ни один из путей не должен использовать `NPM_TOKEN`. В npm Trusted Publisher нужно указать пользователя `avytheone`, репозиторий `effect-dynamic-layer`, файл процесса `release.yml`, не указывать environment и явно разрешить прямую публикацию в npm вместо доступа только к промежуточному выпуску. Настройки издателя нельзя привязать до первой публикации. После неё это можно сделать через npm 11.19.1 или новее без глобального обновления и изменения процесса выпуска: `npm exec --yes --package=npm@11.19.1 -- npm trust github @avytheone/effect-dynamic-layer --file release.yml --repository avytheone/effect-dynamic-layer --allow-publish --yes`; привязку нужно проверить командой `npm trust list @avytheone/effect-dynamic-layer --json`. Настройка пока не выполнялась, а доказательством работы OIDC будет только последующий успешный выпуск по тегу.
