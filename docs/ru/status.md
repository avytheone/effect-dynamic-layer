...
Библиотека реализована и проверена локально. В том числе завершён одобренный прямой переход от строковых идентификаторов узлов к управлению через сервисы `Context` и `LifecycleState`. Предполагаемое имя в npm — `@avytheone/effect-dynamic-layer`, версия — `0.1.0`, но пакет ещё не опубликован, не развёрнут и не проверен в промышленной интеграции. Исходные требования зафиксированы в `HANDOFF-dynamic-layer.md`; владелец отдельно уточнил, что вместо исходной основы Effect 3.x, pnpm и Vitest следует использовать **Effect RC и Bun**.
...
## Текущее состояние подготовки пакета

Проект использует лицензию MIT, правообладатель — Alexey Yakimanskiy, 2026 год. После разрешённой проверки текущих файлов и доступной истории Git репозиторий `avytheone/effect-dynamic-layer` открыт; GitHub подтвердил видимость `PUBLIC`. Пакет готов к отдельно разрешённой первой публикации, но опубликованной версии в реестре npm пока нет. Нельзя утверждать, что `npm install @avytheone/effect-dynamic-layer` работает с реестром, пока такая публикация не подтверждена фактически.

Договор выпуска:

1. Обновить версию в `package.json` и файле блокировки Bun.
2. Выполнить полный набор локальных проверок и зафиксировать проверенное состояние в коммите.
3. Только при наличии разрешения на публикацию отправить точный тег `v${package.version}`.
4. `release.yml`, запускаемый только по тегам `v*`, проверяет точное совпадение тега и версии на GitHub-hosted Ubuntu, использует Bun из `.bun-version`, Node 24 и npm 11.12.1 и повторяет существующие проверки. Он ровно один раз собирает и упаковывает пакет через `npm pack` (`prepack` запускает сборку Bun), а затем проверяет этот самый архив командой `bun run test:package /absolute/path.tgz`.
5. Тот же проверенный файл публикуется командой `npm publish <tarball> --ignore-scripts --access public --provenance --tag <latest|beta>` с меткой `beta` для предварительной версии и `latest` во всех остальных случаях.

Процесс не должен использовать `NPM_TOKEN`. В npm Trusted Publisher нужно указать пользователя `avytheone`, репозиторий `effect-dynamic-layer`, файл процесса `release.yml`, не указывать environment и явно разрешить прямую публикацию в npm вместо доступа только к промежуточному выпуску. Настройки издателя нельзя привязать к пакету до его появления, поэтому первая публикация — отдельная, явно разрешённая операция сопровождающего по действующему для учётной записи правилу 2FA для аутентификации и операций записи; нельзя считать, что она пройдёт без участия человека. После неё можно настроить OIDC; доказательством его работы будет только последующая успешная публикация по тегу.
...
- Не проверялись выполнение в браузере, другие предварительные или основные версии Effect, отдельный путь выполнения в Node и удалённый запуск GitHub Actions. Команды непрерывной интеграции выполнялись только локально; предполагаемый выпуск по тегу через OIDC ещё не запускался.
