...
- Подготовлены метаданные пакета и документация для имени npm `@avytheone/effect-dynamic-layer`, открытого доступа под лицензией MIT и публикации по тегам с подтверждением происхождения через npm OIDC. Версия остаётся `0.1.0`, пакет не опубликован; изменение доступа к репозиторию, первоначальное создание пакета, настройка Trusted Publisher и настоящая публикация по тегу требуют отдельных разрешённых операций и проверки.
