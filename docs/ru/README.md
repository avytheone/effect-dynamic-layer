...
Репозиторий GitHub открыт, проект использует [лицензию MIT](../../LICENSE), правообладатель — Alexey Yakimanskiy, 2026 год. Версия `0.1.0` [опубликована в npm](https://www.npmjs.com/package/@avytheone/effect-dynamic-layer). Контрольная сумма в реестре совпала с проверенным архивом; после новой установки по имени пакета пример README успешно выполнился под Bun и Node.
...
Эта команда выполнена с интерактивным подтверждением 2FA. npm подтвердил создание привязки к GitHub-репозиторию `avytheone/effect-dynamic-layer`, файлу `release.yml`, с правами `publish` и `stage publish`. Не создавайте её заново для каждого выпуска. Для последующей проверки используйте `npm trust list @avytheone/effect-dynamic-layer --json`; npm может потребовать отдельное интерактивное подтверждение 2FA даже для чтения.

Версия `0.1.0` стала разрешённой первой ручной публикацией: проверенный архив отправлен без изменений с авторизацией учётной записи и 2FA, без подтверждения происхождения через CI. После этого настроен Trusted Publisher. Не публикуйте `0.1.0` повторно и не отправляйте её тег в расчёте на повторную публикацию процессом выпуска: версии npm неизменяемы. Полной проверкой выпуска через OIDC станет последующий успешный выпуск по тегу. Публикация другой версии, отправка тега выпуска, изменение доступа к репозиторию и учётных данных по-прежнему требуют явного разрешения на соответствующую операцию.
