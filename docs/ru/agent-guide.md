...
Имя пакета npm — `@avytheone/effect-dynamic-layer`, но версия `0.1.0` ещё не опубликована, а библиотека не заявлена готовой к промышленному использованию. По решению владельца репозиторий GitHub открыт и использует лицензию MIT. Публикация, отправка тега выпуска, развёртывание, изменение доступа к репозиторию и учётных данных по-прежнему требуют соответствующего разрешения.
...
- Перед тегом выпуска обнови версию в `package.json` и `bun.lock`, выполни все локальные проверки и зафиксируй проверенное состояние в коммите. Только после этого сопровождающий с явным разрешением может отправить точный тег `v${package.version}`.
- `release.yml` — путь выпуска на GitHub-hosted Ubuntu, запускаемый только по тегам `v*`. Он должен проверять точное совпадение тега и версии, использовать Bun из `.bun-version`, Node 24 и npm 11.12.1 и выполнять все существующие проверки. Пакет собирается и упаковывается один раз через `npm pack` (`prepack` выполняет сборку Bun), этот самый архив проверяется командой `bun run test:package /absolute/path.tgz`, а затем публикуется как тот же файл через `npm publish <tarball> --ignore-scripts --access public --provenance --tag <latest|beta>`. Для предварительной версии используется `beta`, иначе — `latest`.
- Публикация использует доверенный выпуск npm через OIDC и никогда не использует секрет `NPM_TOKEN`. Поля npm Trusted Publisher: пользователь `avytheone`, репозиторий `effect-dynamic-layer`, файл процесса `release.yml`, без environment; прямую публикацию в npm нужно разрешить явно, а не оставлять доступ только к промежуточному выпуску.
- У нового пакета npm нельзя настроить Trusted Publisher до его создания. Первая публикация — отдельное, явно разрешённое действие сопровождающего по действующему для учётной записи правилу 2FA для аутентификации и операций записи; нельзя считать, что оно пройдёт без участия человека. Только после неё можно настроить издателя и последующим выпуском по тегу проверить OIDC. Нельзя заявлять об успешной установке из реестра, подтверждённом происхождении пакета или публикации через OIDC, пока каждый результат не наблюдался на практике.
